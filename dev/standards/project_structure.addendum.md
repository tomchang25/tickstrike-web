...
`app` may import `platform`: the composition/bootstrap layer is the sanctioned point that constructs platform adapters and injects them into runtime-owned ports (a runtime layer never imports platform, and platform stays a leaf). This edge was added for the settings persistence seam (port_11 child 05) and is reused by later platform-backed features (audio, packaging). `ui` still reaches platform capabilities only through `runtime`, never directly.
