...
The runtime retains the pre-existing Pixi view for each terminal event before projecting the terminal-free snapshot. That retained view becomes a presentation-only ghost, plays the existing terminal timeline, and is destroyed on completion or cancellation. No animation callback mutates `World`, no terminal event gains a sprite payload, and no terminal entity remains in the semantic mirror after the resolved command.
...
- `GameRuntime` receives the resolved event list before it publishes the resulting snapshot. It directs `PresentationDirector` to retain terminal views before `emit()` calls `renderer.updateSnapshot(snapshot)`; retaining after the projection is incorrect because reconciliation would already have destroyed the source view.
- `PresentationDirector` owns only the temporary visual lifetime. It plays existing timelines against retained ghosts and destroys them after completion or cancellation; it never calls `World`, `removeEntity`, or any gameplay mutation.
- `PixiGameRenderer` owns normal snapshot views separately from retained terminal views. Snapshot reconciliation and enemy-presentation datasets operate only on normal views; retained ghosts remain renderable and addressable to terminal timelines but are not projected as live entities.
- Existing `enemy_died`, `enemy_crushed`, `enemy_entered_water`, and `player_died` events carry the entity id and terminal cell required to retain an existing view. `enemy_self_destructed` may accompany `enemy_died` for the same id, so retention and final destruction must deduplicate terminal ids rather than create or destroy two ghosts.
...
- Generation invalidation runs `PresentationDirector.cancel()` before `GameRuntime.loadScenario()` synchronizes the replacement world. Cancellation must destroy every retained ghost and clear position ownership so reset, scenario replacement, and runtime destruction cannot leave an orphan visual.
...
- Retain pre-existing Pixi views as presentation-owned ghosts before snapshot reconciliation and destroy them when their terminal timelines settle or are cancelled.
...
| `src/runtime/GameRuntime.ts`                                    | Small       | Retain terminal presentation views before emitting the terminal-free snapshot.                                               |
| `src/presentation/pixi/PixiGameRenderer.ts`                     | Large       | Separate retained terminal ghosts from snapshot-owned entity views and remove tombstone behavior.                            |
| `src/presentation/timelines/PresentationDirector.ts`            | Medium      | Coordinate ghost retention, terminal timelines, completion cleanup, and cancellation cleanup.                                |
...
3. Add the renderer/director retained-view handoff, then change runtime ordering so terminal views are retained before the terminal-free projection. Route all terminal animation and water motion lookups through the retained view while the ghost exists.
...
- A retained ghost must be moved out of the renderer's snapshot-owned entity-view collection before reconciliation. Keep a dedicated private collection for terminal views; normal `sync`, `updateSnapshot`, enemy-presentation dataset refresh, and snapshot entity reconciliation must not enumerate it as a live actor.
- Renderer lookup methods used by `PresentationDirector` must resolve a retained ghost while its timeline is active. Once released, all lookup methods return no view for that id and the ghost root is destroyed exactly once.
- For water, retain the original enemy presentation so `beginEnteredWater` and authored frame progression continue to use its loaded sprite sheet. The semantic snapshot has no drowning entity during this animation.
...
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Enemy enters water after a nonfatal hit in the same accepted command | Retain one original view, animate the authored water sequence, then destroy one ghost; the entity is absent from the emitted snapshot.                              |
| Bomb self-destruct emits both self-destruct and death events         | Retain and release the entity view once while still presenting the explosion effect.                                                                                |
| Player dies                                                          | Remove the player from the canonical snapshot, retain its view through the defeat timeline, then destroy it without affecting the encounter result UI.              |
| Reset or scenario replacement during a terminal timeline             | Generation cancellation destroys the retained ghost before the replacement snapshot projects; no prior-scene view, position reservation, or dataset entry survives. |
...
3. Reset, scenario replacement, renderer destruction, and interrupted terminal animation leave no retained entity view, position reservation, transient effect, or stale enemy-presentation entry behind.
