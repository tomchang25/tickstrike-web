...
> **Status**: **Phase A 全部完成**（A1–A5、A7 於 2026-07-20；A6 於 2026-07-21）。A6 將 `world.ts` 從 1,658 行拆到 963 行，抽出 `GridBoard`／`CombatOperations`／`WaveRuntime`／`RunBuild` 四個子系統，`world.ts` 留為委派 facade；子系統彼此不得互 import（唯一例外 `combat-operations → grid-board` 取空間答案），`resolveCommittedAttackTransaction` 為唯一跨子系統的 World composer；全單元測試、同 seed determinism harness 與 e2e 綠燈。後續的 engineering hardening——把 layering／子系統／registry 邊界機械化進 `npm run check`、每個 PR 的 GitHub Actions CI、三個 golden determinism gate、`PixiGameRenderer` 拆出 input／preview／board painter、以及 enemy／player／wave 三個 resolution phase 改吃窄能力介面（World facade 已凍結）——已於 2026-07-21 全部完成。character 工作見 [character_featurization_and_viking_split.md](character_featurization_and_viking_split.md)（A6 Step 3 已完成，其 artifact 依賴已解除）。Phase B 仍延後（等 Port 完成 + 更多機制與 Entity 落地後再依 §6.1 雙 gate 決策）
