...
Make every accepted player command expose one authoritative ordered combat-event stream. Remove the split between gameplay-only `events` and optional `semanticEvents` so core, snapshots, runtime, and presentation observe the same result without compatibility fallbacks.

## Summary

The current resolver returns a gameplay-only `events` list while separately constructing `semanticEvents` with the command boundary and Tick-advance events. `World.lastEvents` records the complete list and `GameRuntime` presents it through an optional fallback, so the public result and the actual runtime record disagree.
...
- `resolveCommand()` and its verb-specific resolvers own construction of the command result. `finishAccepted()` must return the one complete ordered stream as `ActionResolution.events`; it must not retain `semanticEvents` or construct a second public view.
...
- `GameRuntime.drainCommands()` calls the resolver, publishes the resulting World snapshot, and passes `resolution.events` directly to `PresentationDirector.play()`. It must remove the optional fallback because no alternate result stream remains.
...
- Do not introduce a compatibility alias, optional fallback, event adapter, or a second gameplay-only collection. Consumers that need a subset may filter the one authoritative stream locally.

## Scope

### Included

- Replace the dual `events` and `semanticEvents` result contract with one complete non-optional `events` stream.
...
1. Change the resolver result contract and `finishAccepted()` so the existing complete ordered sequence is returned only as `events`, then remove `semanticEvents` from accepted and rejected result shapes.
2. Change `GameRuntime` to pass the unified result stream directly to presentation while preserving snapshot publication before timeline playback.
3. Update focused resolver and enemy lifecycle tests to assert boundary events, direct outcomes, enemy-phase outcomes, World snapshot equality, and rejected-command non-mutation through the single field.
4. Update the existing browser event-log/debug assertion to prove the visible and debug-observed sequence remains complete and ordered, then run the required focused verification.

## Implementation Notes

- Preserve `events` as the public property name. It changes from a gameplay-only compatibility view to the complete authoritative stream; no renamed replacement field is needed.
