...
Add a compact horizontal order rail at the bottom center of the playfield, clear of the existing corner controls. The Player token leads, followed by one token per living enabled enemy in stable spawn order. Tokens use the existing dark HUD language with a cyan active ring, dimming for inactive enemies, and accessible labels; normal state has no badge, while exceptional state uses an explicit text badge (`STG`, `REC`, or `REST`) or an attack-warning symbol. No new art pipeline is introduced.
...
Default `staggered` pacing hands the logical active highlight to the next visible slot after 0.1 seconds while prior VFX may continue. `wait-for-vfx` hands off only after the current slot's VFX settles. No-op slots publish their post-slot badge state and finish within one render turn. A new input finishes active VFX, drains every unstarted slot, settles the bar, and preserves the existing command-queue fast-forward contract.

Settings schema v4 adds the pacing choice, defaults old payloads to `staggered`, and preserves every v1-v3 compatibility path. Hovering or focusing a HUD token highlights the matching arena entity; hovering an occupied arena cell highlights its token. Reduced-motion presentation keeps the same information and pacing while removing non-essential token transitions.
...
- `PresentationDirector` keeps VFX lifetime. The controller owns logical handoff and whole-turn fast-forward; staggered handoff clears the previous active highlight after 0.1 seconds even while its VFX continues.
...
- `SettingsStore` v4 owns `turnOrderPacing`; every v1-v3 path stays supported and defaults absent/invalid values to `staggered`.
...
- Reactive order bar, badges, removal/refill, and HUD/arena cross-highlight.
- Both pacing modes, no-op handling, fast-forward, reduced motion, Settings v4, and focused evidence.
...
| `src/runtime/{turn-order-controller,game-runtime,settings-store}.ts`   | Large       | Own playback, integration, fast-forward, and Settings v4         |
...
4. Add Settings v4, session wiring, HUD/settings UI, CSS, focus semantics, and reduced motion.
...
- The labelled ordered list is fixed at the bottom center of the playfield and uses focusable 46px tokens, short exceptional-state badges, accessible names, a cyan active/focus outline, and a reduced-motion-safe completion transition. Normal state renders no badge; ambiguous wave-like labels such as `W1` are prohibited.
...
| VFX overlap in staggered mode          | Only the latest logical slot stays active                             |
...
| Stored pacing is absent/invalid        | Use `staggered` and retain older settings                             |
...
2. Slot events and status changes play in canonical order; active HUD and arena highlights agree, completed slots leave the segment, and the next segment refills at the tick boundary.
3. Default playback starts visible slots 0.1 seconds apart with overlap, wait-for-VFX serializes them, and no-op slots add no perceptible delay.
...
6. The pacing preference survives reload through Settings v4 while v1-v3 and malformed payloads retain safe defaults.
