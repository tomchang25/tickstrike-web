...
Execute top to bottom, one child per session, tests green before each commit. Shipped rows are retained with their spec named in plain text but no link, per `dev/workflows/closeout_standard.addendum.md`. Child 01 was a test-hygiene move (no changelog entry); children 02–03a are recorded in `CHANGELOG.md`.
