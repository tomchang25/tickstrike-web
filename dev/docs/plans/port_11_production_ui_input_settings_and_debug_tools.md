...
Execute top to bottom, one child per session, tests green before each commit. Children 01 (e2e spec split, a test-hygiene move with no changelog entry) and 02 (windup cancel; see `CHANGELOG.md`) shipped; their specs are archived.
