...
Execute top to bottom, one child per session, tests green before each commit. Child 01 shipped as a test-hygiene move (no changelog entry per its process-only nature); its spec is archived.
