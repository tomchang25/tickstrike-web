...
Execute top to bottom, one child per session, tests green before each commit. Shipped rows are retained with their spec named in plain text but no link, per `dev/workflows/closeout_standard.addendum.md`. Child 01 was a test-hygiene move (no changelog entry); children 02–03 are recorded in `CHANGELOG.md`.
...
| 03a Keyboard bump-attack          | A WASD/arrow movement key attacks instead of moving when its direction has an attack target (`hasTarget`); IJKL explicit attack and Alt+cursor Mobility are retained unchanged                                                                                                                           | [port_11_03a_keyboard_bump_attack.implementation_spec.md](port_11_03a_keyboard_bump_attack.implementation_spec.md) |
