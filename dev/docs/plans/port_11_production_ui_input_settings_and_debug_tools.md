...
2. Bind keyboard input to Move (WASD/arrows) and Normal Attack (IJKL); a movement key whose direction holds an attack target performs the attack instead of moving. Dash and Smash stay unified as the pointer-driven Mobility mode entered with Alt, with no keyboard binding. There is no Wait command (a whiffed IJKL Normal Attack is the deliberate pass-a-tick option) and no dedicated restart input: restart is offered only by terminal overlays and, later, the settings panel.
...
Execute top to bottom, one child per session, tests green before each commit. Shipped rows are retained with their spec named in plain text but no link, per `dev/workflows/closeout_standard.addendum.md`. Child 01 was a test-hygiene move (no changelog entry); children 02–04 are recorded in `CHANGELOG.md`.
...
| 03 Input layer                    | Extract keyboard handling from `use-game-session.ts` into `src/ui/input/` (pure keymap + driver hook) with no new bindings; blur/visibility/route release including the stuck Alt pointer mode; right-click during an armed windup dispatches the child 02 cancel (contextmenu suppressed on the canvas) | Shipped — port_11_03_input_layer.implementation_spec.md                                                                    |
| 03a Keyboard bump-attack          | A WASD/arrow movement key attacks instead of moving when its direction has an attack target (`hasTarget`); IJKL explicit attack and Alt+cursor Mobility are retained unchanged                                                                                                                           | Shipped — port_11_03a_keyboard_bump_attack.implementation_spec.md                                                          |
| 04 Production HUD                 | Game-style overlay HUD on the board (player HP/mobility, wave, control hints, build bar with a Build overview) in both shells, pointer-events passing through to the board; consistency baseline only, parity deferred to port_13                                                                        | Shipped — port_11_04_production_hud.implementation_spec.md                                                                 |
| 05 Settings and debug gating      | localStorage-backed settings through a platform adapter with a runtime owner (v1: DEV-only debug overlay toggle); a settings-panel whole-run Restart through the existing reset entrance; production build keeps excluding all debug code; widens app→platform for the persistence seam                  | [port_11_05_settings_and_debug_gating.implementation_spec.md](port_11_05_settings_and_debug_gating.implementation_spec.md) |
