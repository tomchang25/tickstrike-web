...
3. Every enemy role receives a Dash-kill terminal animation in which the body is cut in half, authored per direction through the offline sprite animation pipeline as a recipe over that role's existing base sheet. This lands in child 13.3 on the existing terminal animation lifecycle (moved from 13.4 by the later entity-versus-world ownership decision); Smash-kill terminal presentation is deferred to `TODO.md`.
...
| 13.3  | Player and enemy visual profiles                               | Shipped — all entity visual-profile children (13.3a–13.3e)                                                                                                                             |
...
Child 13.3 shipped every entity-attached animation surface across five children: 13.3a shared drop shadow and 13.3b ranged redraw plus rescale, 13.3c player attack/weapon and dash states, 13.3d Bomb and Charge prepare/execute body animations, and 13.3e Dash-kill body-split terminals with the minimal death-cause semantics. The ownership boundary with 13.4 is entity versus world: animation on an entity's own body belongs to 13.3, effects over cells, paths, or the board to 13.4. Authored and derived sheets carried mandatory human approval gates.
