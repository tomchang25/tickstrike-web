...
Execute top to bottom, one child per session, tests green before each commit. Shipped rows are retained with their spec named in plain text but no link, per `dev/workflows/closeout_standard.addendum.md`. All children have shipped and this plan is complete. Child 01 was a test-hygiene move (no changelog entry); children 02–05 are recorded in `CHANGELOG.md`.
