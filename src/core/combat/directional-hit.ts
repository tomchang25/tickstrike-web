...
export function classifyHitAngle(
  ...
  if (!facing || !relationship || sameCell(attackerCell, targetCell)) return undefined;

  if (relationship.x === facing.x && relationship.y === facing.y) return "front";
  if (relationship.x === -facing.x && relationship.y === -facing.y) return "back";
  return "side";
}
...
export function calculateDirectionalHit(input: DirectionalHitInput): DirectionalHitResult | undefined {
  if (input.target.phase !== "alive") return undefined;
  const angle = classifyHitAngle(input.attackerCell, input.target.cell, input.target.facing);
  if (!angle || !Number.isFinite(input.damage) || input.damage <= 0) return undefined;
  ...
  const guardDamage = protectionTicks > 0 && rawGuardDamage > 0
  ...
  return {
    ...
    guardBroken,
    staggerBurst,
    feedback: input.target.activity === "staggered"
      ? "staggered"
      : guardBroken
        ? "guard_break"
        : guardBefore > 0
