...
Use the narrowest validation layer that proves the requested change. Run `npm test` for unit behavior, `npm run build` for the production build, `npm run test:e2e` for browser-visible behavior, or `npm run verify` for the canonical non-browser verification. Do not run browser tests unless browser coverage is required. Report every command run, its result, and any verification gap.
